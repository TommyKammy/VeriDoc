...
- Criteria source Git blob:
  `4b6ec7a3aa107388b0fc45a75ee12818bf4af252`
- Evaluator Git blob:
  `4bb380a26bb5c1d8cc2354505ad57678260934e2`
- Generated at: `2026-07-26` (Asia/Tokyo)
...
- Dataset manifest SHA-256:
  `eb92e93d1d6be6ae846f43cf84b2355df909827e7da10b51c45df2623b9ed6d1`
...
| AC-REVIEW | 達成 / pass | The manifest independently declares one scanned-PDF high-risk target. The snapshot detects and covers it at the accepted OCR boundary with zero misses and zero auto-confirms; outside that boundary, an `authoritative review decision is required`. Missing, malformed, or unmatched gold targets reject the rollup. | `none` | — | Preserve the independent target set, high-risk guards, and fail-closed rollup. | — |
| AC-EFFICIENCY | 未達 / fail | The validator-backed protocol and `docs/mvp-scope-decisions.md` pin source fixtures, target formats, common timed instructions, arm-specific tool/assistance contracts, and the ordered completion checklist, use opaque UUIDv4 approval references, and bind every assessment to immutable task, checklist, and gold case digests. The validator reports timing reductions for calculable completed pairs while keeping them out of the median because execution remains `unverified_validation_only`. The pinned structured scoring package is explicitly `unapproved_validation_only`, the approved `p12g-02-v1` manifest predates those targets, and no completed participant record exists. | `human_evidence_gap` | manual | Approve authenticated execution-attestation integration and a new manifest/decision revision with structured high-risk targets, then run the fixed protocol with a real pseudonymous cohort. | P12G-14 |
...
| FC-HIGH-RISK | 達成 / pass | Component/browser guards cover actual high-risk targets; the accepted five-case snapshot records one manifest-owned target, one covered target, zero misses, and zero auto-confirms. | `none` | — | Preserve the high-risk gold target and fail-closed miss calculation. | — |
...
| EM-USER-REVIEW | 未達 / fail | The versioned protocol, closed privacy-safe schema, pinned practice, timed-task, completion-checklist, and gold packages, synthetic valid/invalid examples, and P12G-14 handoff bind each participant to opaque consent evidence and each run to manifest target format, exact case instructions, assigned-arm assistance contract, shared ordered checklist content, per-case checklist and gold digests, UUIDv4 participant/sealed-record identities, strict timing, hidden-gold deviations, misses, blockers, retries, exclusions, and total/per-arm evidence. Calculable completed pairs retain their measurements without becoming median-eligible. Renewed structured-target approval and a real reviewer record remain absent. | `human_evidence_gap` | manual | Approve the structured targets, then execute `docs/mvp-human-review-execution-checklist.md` and retain a validator-passing completed record. | P12G-14 |
...
| OD-TEMPLATES | 未達 / fail | `TommyKammy` approved the five-case contract at product commit `584ef2db12a6676abb65f75de1ec38145e06b487`, but the fixture expectations changed afterward without a new decision revision. The evaluator pins the contract derived from the approved commit and rejects the current drift. | `decision_gap` | manual | Issue a new decision revision and approval for the current manifest, fixture expectations, and selected fixture contents. | P12G-02 |
...
| P12G-13 | hybrid | P12G-02 | `p12g-13-human-review-v1`, its strict duplicate-key-rejecting privacy-safe schema/validator, immutable practice/task/checklist/gold packages, approved manifest/fixture/build pins, opaque study/run/artifact IDs, sealed artifact digests, strict RFC 3339 timestamps, synthetic validation vectors, and the P12G-14 execution checklist are fixed; structured-target reapproval and real participant execution remain manual. |
