...
- Criteria source Git blob:
  `bbaa69f5ba200768579652d3c6834c18df289d09`
- Evaluator Git blob:
...
- Report result: `item_count=20`, unique item IDs `20`,
  `overall_decision=fail` (`pass=2`, `fail=18`)
...
| AC-TEMPLATE | 一部達成 / fail | The versioned manifest fixes five categories and Word, Excel, text PDF, and record PDF pass, but the remaining scanned-PDF case fails and the representative scope is not authoritatively approved. | `e2e_gap`, `decision_gap` | manual/hybrid then codex | Approved 3–5-template scope plus passing results from that manifest revision. | P12G-02, P12G-12 |
...
| AC-PROVENANCE | 一部達成 / fail | The browser E2E now binds source page/bbox through review, artifact, and audit evidence under one correlation ID; no five-case source-link coverage result yet proves 95%+. | `measurement_gap` | codex | Snapshot-consistent five-case source-link metric. | P12G-12 |
