...
- Criteria source Git blob:
  `fb5ebe0dc29f8e48700834c9f99e35d4fa72445c`
- Evaluator Git blob:
...
| AC-EFFICIENCY | 未達 / fail | The validator-backed protocol and `docs/mvp-scope-decisions.md` pin source fixtures and target formats to the approved manifest, use opaque UUIDv4 approval references, and bind every assessment to immutable package/case gold digests. The validator reports timing reductions for calculable completed pairs while keeping them out of the median because execution remains `unverified_validation_only`. The pinned structured scoring package is explicitly `unapproved_validation_only`, the approved `p12g-02-v1` manifest predates those targets, and no completed participant record exists. | `human_evidence_gap` | manual | Approve authenticated execution-attestation integration and a new manifest/decision revision with structured high-risk targets, then run the fixed protocol with a real pseudonymous cohort. | P12G-14 |
...
| EM-USER-REVIEW | 未達 / fail | The versioned protocol, closed privacy-safe schema, pinned practice and gold packages, synthetic valid/invalid examples, and P12G-14 checklist bind each participant to opaque consent evidence and each run to manifest target format, exact per-case gold content, UUIDv4 participant/sealed-record identities, strict timing, hidden-gold deviations, miss, over-detection, completion/blocker, retry, exclusion, and total/per-arm evidence. Calculable completed pairs retain their measurements without becoming median-eligible. Renewed structured-target approval and a real reviewer record remain absent. | `human_evidence_gap` | manual | Approve the structured targets, then execute `docs/mvp-human-review-execution-checklist.md` and retain a validator-passing completed record. | P12G-14 |
