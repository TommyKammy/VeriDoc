...
- Criteria source Git blob:
  `6fb79f85eaaccbfbdc7b8913ac5d662fe025ac38`
- Evaluator Git blob:
...
| AC-EFFICIENCY | 未達 / fail | `docs/mvp-human-review-protocol.md`, its validator-backed schema, and `docs/mvp-scope-decisions.md` pin the approved product commit/tree, manifest contract, source fixtures, strict timestamps, per-participant consent attestations, an immutable per-arm practice package, independently generated UUIDv4 participant IDs, one shared checklist revision, sealed artifact digests, an honestly retained hidden-gold boundary, and paired calculation. Current per-run provenance binds only the approved source-tree listing and marks execution `unverified_validation_only`, so completed-study pairs remain ineligible and no median is claimed. The approved `p12g-02-v1` manifest also predates structured high-risk targets, and no completed participant record exists. | `human_evidence_gap` | manual | Approve authenticated execution-attestation integration and a new manifest/decision revision with structured high-risk targets, then run the fixed protocol with a real pseudonymous cohort. | P12G-14 |
...
| EM-USER-REVIEW | 未達 / fail | The versioned protocol, closed privacy-safe schema, synthetic valid/invalid examples, and P12G-14 checklist bind each participant to a consent attestation and immutable per-arm practice package and each run to an independently generated UUIDv4 participant ID, the shared checklist revision, the approved source fixture/build, and a unique sealed-artifact record while defining strict timing, honestly retained hidden-gold deviations, miss, over-detection, completion/blocker, retry, exclusion, and total/per-arm evidence. Renewed structured-target approval and a real reviewer record remain absent. | `human_evidence_gap` | manual | Approve the structured targets, then execute `docs/mvp-human-review-execution-checklist.md` and retain a validator-passing completed record. | P12G-14 |
