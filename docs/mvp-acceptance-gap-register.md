...
- Criteria source Git blob:
  `660cdb65100a97adade458da126cfd75b9833580`
- Evaluator Git blob:
...
| AC-TEMPLATE | 一部達成 / fail | The versioned manifest fixes five categories, but the harness has zero passing cases and the representative scope is not authoritatively approved. | `e2e_gap`, `decision_gap` | manual/hybrid then codex | Approved 3–5-template scope plus passing results from that manifest revision. | P12G-02, P12G-12 |
| AC-QUALITY | 未達 / fail | Word and Excel are `unknown`; text, scanned, and record PDF are `fail`; no per-template 80%+ metric can be claimed. | `implementation_gap`, `e2e_gap` | codex/hybrid | Fix or explicitly validate all three PDF boundaries, then publish per-template cell/content agreement. | P12G-05, P12G-06, P12G-07, P12G-12 |
...
| AC-REVIEW | 一部達成 / fail | High-risk guards have component tests, but every case has `review_decision=null`; Word/Excel review evaluation is `unknown`. | `implementation_gap`, `e2e_gap`, `human_evidence_gap` | codex then manual | Authoritative reviewer/approver decisions, keyboard review flow, and zero high-risk misses across the accepted dataset. | P12G-04, P12G-08, P12G-12 |
...
| EM-E2E | 一部達成 / fail | A repo-owned browser run now emits screenshots, trace, API result, downloaded artifact, audit artifact, and correlation metadata for one representative case; the five-case report remains three `fail`, two `unknown`, and zero `pass`. | `implementation_gap`, `e2e_gap` | codex/hybrid | P12G-04 through P12G-12 evidence from one committed snapshot with all intended cases passing. | P12G-04–P12G-12 |
...
| --- | --- | --- | --- | --- | --- |
| mvp-word-001 | Word | `unknown` | artifact/pass; audit/pass; input_size/pass; processing_time/pass; timeout/pass; review/unknown | `no authoritative reviewer decision was recorded by the conversion run` | absent (`null`) |
| mvp-excel-001 | Excel | `unknown` | artifact/pass; audit/pass; input_size/pass; processing_time/pass; timeout/pass; review/unknown | `no authoritative reviewer decision was recorded by the conversion run` | absent (`null`) |
...
| mvp-scanned-pdf-001 | scanned PDF | `fail` | artifact/fail; audit/pass; input_size/pass; processing_time/pass; timeout/pass; review/unknown | `artifact content validation is unavailable for scanned_pdf_ocr`; no authoritative reviewer decision was recorded. | absent (`null`) |
| mvp-record-pdf-001 | record PDF | `fail` | artifact/fail; audit/pass; input_size/pass; processing_time/pass; timeout/pass; review/unknown | `artifact content validation is unavailable for pdf_to_word`; no authoritative reviewer decision was recorded. | absent (`null`) |
