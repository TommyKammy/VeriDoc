...
- Criteria source Git blob:
  `b6f9ae42cc926601f52bb394b0adcdca80473d87`
- Evaluator Git blob:
...
| AC-EFFICIENCY | 未達 / fail | `docs/mvp-human-review-protocol.md`, its validator-backed schema, and `docs/mvp-scope-decisions.md` pin the approved product commit/tree, manifest contract, source fixtures, per-run build provenance, strict timestamps, opaque identifiers, sealed artifact digests, hidden-gold boundary, and paired median. The approved `p12g-02-v1` manifest predates structured high-risk targets, so `efficiency_target_met` remains false; no completed participant record exists. | `human_evidence_gap` | manual | Approve a new manifest/decision revision with structured high-risk targets, then run the fixed protocol with a real pseudonymous cohort. | P12G-14 |
