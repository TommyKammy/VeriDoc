...
| AC-PERFORMANCE | 性能: 代表文書の処理時間・サイズ・timeout基準を守る | [#285](https://github.com/TommyKammy/VeriDoc/issues/285), [#289](https://github.com/TommyKammy/VeriDoc/issues/289) | `datasets/mvp_evaluation_manifest_v1.json`, `docs/mvp-performance-limits.md`, `tests/test_evaluate_dataset.py`, `tests/test_poc_web_api.py`; final evidence: #289 harness output | **一部達成** — 10秒の処理時間目標、2 MiB上限、30秒timeoutと超過時のfail-closed表示は定義・テスト済み。最終代表runの測定記録は #289 で確定する。 |
