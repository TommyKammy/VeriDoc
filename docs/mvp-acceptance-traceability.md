...
| AC-AUTH | 認証: 簡易認証＋ロールが機能 | [#276](https://github.com/TommyKammy/VeriDoc/issues/276), [#277](https://github.com/TommyKammy/VeriDoc/issues/277), [#308](https://github.com/TommyKammy/VeriDoc/issues/308) | `docs/mvp-scope-decisions.md`, `tests/test_desktop_api_auth.py`, `tests/test_poc_web_api.py`; current gap: P12G-10 | **一部達成** — the six-role permission matrix and MVP/Phase13 boundary are approved, but unset `VERIDOC_LOCAL_AUTH_TOKENS` still permits unauthenticated non-approval operations and approval does not yet require a preceding distinct-actor review/edit event; mandatory-auth, workflow, deny-path, and token-lifecycle E2E evidence remains open. |
...
6. The evaluator must recompute the approved manifest and permission-matrix
   input contracts. Any drift fails the affected `OD-*` item until a renewed
   human-approved decision revision updates the recorded pins.
