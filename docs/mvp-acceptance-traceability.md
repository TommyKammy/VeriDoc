...
items with `overall_decision=fail`, and its five-case harness contains zero `fail`, zero `unknown`, and five `pass` results.
...
| AC-TEMPLATE | テンプレート: 代表3〜5種で変換が成立 | [#283](https://github.com/TommyKammy/VeriDoc/issues/283), [#284](https://github.com/TommyKammy/VeriDoc/issues/284), [#308](https://github.com/TommyKammy/VeriDoc/issues/308), [#312](https://github.com/TommyKammy/VeriDoc/issues/312), [#318](https://github.com/TommyKammy/VeriDoc/issues/318) | `datasets/mvp_evaluation_manifest_v1.json`, `docs/mvp-scope-decisions.md`, `tests/test_dataset_fixtures.py`, `tests/test_evaluate_dataset.py`, `evidence_snapshot.metrics_rollup.dimensions.completeness` | **達成** — decision revision `p12g-02-v1` adopts the five-category manifest, and one fail-closed snapshot contains exactly one passing structured result for Word, Excel, text PDF, scanned PDF, and record PDF. The scanned case remains an explicit OCR block and does not claim fabricated text. |
| AC-QUALITY | 品質: 主要帳票でセル一致率80%+の目安 | [#283](https://github.com/TommyKammy/VeriDoc/issues/283), [#284](https://github.com/TommyKammy/VeriDoc/issues/284), [#289](https://github.com/TommyKammy/VeriDoc/issues/289), [#312](https://github.com/TommyKammy/VeriDoc/issues/312), [#318](https://github.com/TommyKammy/VeriDoc/issues/318) | `tests/test_evaluate_dataset.py`, `evidence_snapshot.metrics_rollup.dimensions.quality`, per-case `metrics.quality` | **達成** — the accepted snapshot reports 56/56 cell/content matches (100%, threshold 80%): Word 9 cells, Excel 29 table cells, text PDF 12 cells, one scanned explicit-review block, and five record-PDF body blocks. The scanned contribution is explicitly block agreement, not OCR text accuracy. |
| AC-PROVENANCE | 追跡性: 出典紐づけ率95%+目安、要確認から原本へ辿れる | [#278](https://github.com/TommyKammy/VeriDoc/issues/278), [#284](https://github.com/TommyKammy/VeriDoc/issues/284), [#289](https://github.com/TommyKammy/VeriDoc/issues/289), [#315](https://github.com/TommyKammy/VeriDoc/issues/315), [#318](https://github.com/TommyKammy/VeriDoc/issues/318) | `tests/test_document_ir_v1.py`, `tests/test_web_pdf_preview.py`, `tests/test_evaluate_dataset.py`, `tests/test_mvp_browser_e2e.py`, `evidence_snapshot.metrics_rollup.dimensions.provenance` | **達成** — the accepted snapshot reports 12/12 source bindings (100%, threshold 95%): five audit source-hash bindings plus one text-PDF source comment, one scanned block link, and five record-PDF block links. Browser evidence separately preserves original page/bbox navigation. |
| AC-REVIEW | レビュー: 高リスク項目の自動確定ゼロ・見逃し率0 | [#278](https://github.com/TommyKammy/VeriDoc/issues/278), [#284](https://github.com/TommyKammy/VeriDoc/issues/284), [#289](https://github.com/TommyKammy/VeriDoc/issues/289), [#314](https://github.com/TommyKammy/VeriDoc/issues/314), [#318](https://github.com/TommyKammy/VeriDoc/issues/318) | `tests/test_automatic_validation.py::test_high_risk_item_cannot_be_auto_confirmed_even_when_value_matches`, `tests/test_template_fingerprint.py::TemplateFingerprintTest::test_high_risk_field_requires_review_when_matrix_omits_review_level`, `tests/test_poc_web_api.py::test_convert_uploaded_document_requires_review_for_high_risk_template_field`, `tests/test_mvp_browser_e2e.py`, `tests/test_evaluate_dataset.py`, `evidence_snapshot.metrics_rollup.dimensions.high_risk` | **達成** — the manifest independently declares one scanned-PDF high-risk target; the accepted snapshot detects and covers that target at the OCR boundary with zero misses and zero auto-confirms. Cases with an explicit empty target set remain `no_targets`; missing, malformed, or unmatched gold targets reject the rollup. |
| AC-EFFICIENCY | 効率: 人手修正時間の削減を測定し効果を確認（30%+目安） | [#284](https://github.com/TommyKammy/VeriDoc/issues/284), [#285](https://github.com/TommyKammy/VeriDoc/issues/285), [#289](https://github.com/TommyKammy/VeriDoc/issues/289), [#308](https://github.com/TommyKammy/VeriDoc/issues/308), [#319](https://github.com/TommyKammy/VeriDoc/issues/319) | `docs/mvp-human-review-protocol.md`, `docs/mvp-human-review-practice-package.json`, `docs/mvp-human-review-timed-task-package.json`, `docs/mvp-human-review-completion-checklist-package.json`, `datasets/mvp_human_review_gold_package_v1.json`, `docs/mvp-human-review-evidence.schema.json`, and `scripts/ci/validate_mvp_human_review_evidence.py`; current gaps: authenticated execution attestation, renewed structured-target approval, and real participant comparison | **未達** — the protocol pins the approved product commit/tree, manifest source fixtures and target formats, common timed instructions plus arm-specific assistance contracts, privacy-safe opaque approval references, immutable practice/task/checklist/gold package digests, strict timestamps, UUIDv4 participant IDs, case-bound checklist and sealed artifact digests, a hidden-gold boundary, and paired calculation. Completed records retain calculable pair measurements, but current provenance remains `unverified_validation_only`, so those pairs are ineligible and no median is claimed. The pinned structured scoring content is explicitly `unapproved_validation_only`; the approved `p12g-02-v1` manifest predates it, and no completed human record exists. |
| AC-PERFORMANCE | 性能: 代表文書の処理時間・サイズ・timeout基準を守る | [#285](https://github.com/TommyKammy/VeriDoc/issues/285), [#289](https://github.com/TommyKammy/VeriDoc/issues/289), [#318](https://github.com/TommyKammy/VeriDoc/issues/318) | `datasets/mvp_evaluation_manifest_v1.json`, `docs/mvp-performance-limits.md`, `tests/test_evaluate_dataset.py`, `evidence_snapshot.metrics_rollup.dimensions.performance` | **達成** — one snapshot retains every case result and the existing 10-second processing, 2 MiB input, and 30-second timeout limits; all 15 case/dimension observations pass, and inconsistent, missing, or unknown limits fail closed. |
...
| --- | --- | --- | --- | --- |
| FC-HIGH-RISK | 高リスク項目が自動確定される、または要確認に乗らない | [#278](https://github.com/TommyKammy/VeriDoc/issues/278), [#289](https://github.com/TommyKammy/VeriDoc/issues/289), [#318](https://github.com/TommyKammy/VeriDoc/issues/318) | `tests/test_automatic_validation.py::test_high_risk_item_cannot_be_auto_confirmed_even_when_value_matches`, `tests/test_template_fingerprint.py::TemplateFingerprintTest::test_high_risk_field_requires_review_when_matrix_omits_review_level`, `tests/test_poc_web_api.py::test_convert_uploaded_document_requires_review_for_high_risk_template_field`, `tests/test_mvp_browser_e2e.py`, `tests/test_evaluate_dataset.py`, `evidence_snapshot.metrics_rollup.dimensions.high_risk` | **達成** — component and browser guards force actual high-risk targets through `requires_review`; the manifest-owned gold denominator contains one scanned-PDF target and the snapshot records one covered target, zero misses, and zero auto-confirms. Omitting the emitted item or its gold declaration makes the metric non-passing. |
...
| --- | --- | --- | --- | --- |
| EM-USER-REVIEW | 実務担当者によるレビュー試行（時間・見逃し・過検出） | [#283](https://github.com/TommyKammy/VeriDoc/issues/283), [#284](https://github.com/TommyKammy/VeriDoc/issues/284), [#289](https://github.com/TommyKammy/VeriDoc/issues/289), [#308](https://github.com/TommyKammy/VeriDoc/issues/308), [#319](https://github.com/TommyKammy/VeriDoc/issues/319) | `docs/mvp-human-review-protocol.md`, `docs/mvp-human-review-practice-package.json`, `docs/mvp-human-review-timed-task-package.json`, `docs/mvp-human-review-completion-checklist-package.json`, `datasets/mvp_human_review_gold_package_v1.json`, `docs/mvp-human-review-execution-checklist.md`, and validator-backed valid/invalid examples | **未達** — the execution protocol records opaque consent/quality references, immutable practice, timed-task, completion-checklist, and gold packages, case and arm assistance digests, manifest-bound target formats, per-case checklist and gold digests, correction time even for calculable ineligible pairs, hidden-gold violations, source-tree provenance, sealed-artifact identities, misses, blockers, retries, exclusions, and total/per-arm metrics under UUIDv4 participant IDs. Authenticated execution evidence, renewed structured-target approval, and a real participant record remain absent. |
| EM-E2E | 代表テンプレートでのE2E評価 | [#283](https://github.com/TommyKammy/VeriDoc/issues/283), [#284](https://github.com/TommyKammy/VeriDoc/issues/284), [#289](https://github.com/TommyKammy/VeriDoc/issues/289), [#309](https://github.com/TommyKammy/VeriDoc/issues/309), [#312](https://github.com/TommyKammy/VeriDoc/issues/312), [#315](https://github.com/TommyKammy/VeriDoc/issues/315), [#316](https://github.com/TommyKammy/VeriDoc/issues/316) | Versioned five-case manifest and current report plus `artifacts/mvp-browser-e2e/<run-id>/evidence.json`; current gap: P12G-11/P12G-12 | **一部達成** — one repo-owned browser run packages screenshots, trace, API result, downloaded artifact, audit artifact, complete audit chains, role/token/segregation evidence, and a correlated fail-closed acceptance snapshot; all five dataset cases now pass, while the retained rerun package and accepted-scope aggregate remain open. |

## Open Decisions

| ID | 15.3 open item | Owner issue / phase | Evidence needed to close | Current status |
| --- | --- | --- | --- | --- |
| OD-TEMPLATES | 対象業務・テンプレートの確定 | Historical owner [#283](https://github.com/TommyKammy/VeriDoc/issues/283); decision [#308](https://github.com/TommyKammy/VeriDoc/issues/308) | `datasets/mvp_evaluation_manifest_v1.json`, `docs/mvp-scope-decisions.md`; evaluator validates the approved manifest contract hash | **未達** — `TommyKammy` approved the five-case contract at product commit `584ef2db12a6676abb65f75de1ec38145e06b487` on `2026-07-22`, but fixture expectations changed afterward. The current contract remains unapproved until the decision owner supplies a new revision and approval. |
