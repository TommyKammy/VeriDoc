...
`VERIDOC_LOCAL_AUTH_TOKENS` is configured. Use comma-separated `role=token`
entries:

```bash
VERIDOC_LOCAL_AUTH_TOKENS='viewer=<viewer-token>,reviewer=<reviewer-token>,approver=<approver-token>,admin=<admin-token>' python3 -m services.api.poc_web
```
